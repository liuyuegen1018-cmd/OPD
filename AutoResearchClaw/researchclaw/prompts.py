...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "code_generation": {
        ...
        "user": (
            ...
            "line)\n"
            "- main.py MUST write `metrics.json`, `best_score.json`, and "
            "`results.json` before exiting. `metrics.json` must contain a "
            "top-level `metrics` object with finite numeric values and "
            "`best_score.json` must contain `metric`, `score`, `model`, and "
            "`details` fields. If valid metrics cannot be computed, exit "
            "non-zero rather than producing empty metrics.\n"
