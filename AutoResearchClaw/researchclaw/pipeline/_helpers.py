...
def _flatten_numeric_metrics(value: Any, *, prefix: str = "") -> dict[str, float]:
    """Extract finite scalar metrics from nested experiment artifacts.

    This intentionally accepts several common shapes used by generated code:
    top-level scalars, ``metrics`` dicts, per-condition dicts, and MAARS-style
    ``best_score.json`` payloads.
    """
    metrics: dict[str, float] = {}
    if isinstance(value, dict):
        if isinstance(value.get("score"), (int, float)) and value.get("metric"):
            score = float(value["score"])
            if math.isfinite(score):
                key = str(value["metric"])
                metrics[prefix + key if prefix else key] = score
        for key, child in value.items():
            ...
            if isinstance(child, (int, float)):
                ...
                if math.isfinite(val):
                    metrics[next_prefix] = val
                    if is_metric_name(str(key)):
                        metrics.setdefault(str(key), val)
            elif isinstance(child, (dict, list, tuple)):
                metrics.update(_flatten_numeric_metrics(child, prefix=next_prefix))
...
def _select_primary_metric(
    ...
    for name in preferred_names:
        if not name:
            continue
        if name in finite:
            return {"name": name, "score": finite[name], "direction": metric_direction}
        suffix_matches = {
            key: val
            for key, val in finite.items()
            if key.endswith(f"/{name}") or key.endswith(f"/{name}_mean")
        }
        if suffix_matches:
            chooser = min if metric_direction == "minimize" else max
            key = chooser(suffix_matches, key=suffix_matches.get)
            return {"name": key, "score": suffix_matches[key], "direction": metric_direction}

    chooser = min if metric_direction == "minimize" else max
    key = chooser(finite, key=finite.get)
    return {"name": key, "score": finite[key], "direction": metric_direction}


def _write_metrics_contract(
    target_dir: Path,
    metrics: dict[str, Any],
    *,
    metric_key: str = "",
    ...
    """Write canonical metrics artifacts consumed by later pipeline stages."""
    target_dir.mkdir(parents=True, exist_ok=True)
    flat = _flatten_numeric_metrics(metrics)
    if not flat:
        flat = {
            key: float(value)
            for key, value in metrics.items()
    ...
    primary = _select_primary_metric(flat, metric_key, metric_direction)
    contract: dict[str, Any] = {
        "schema_version": "researchclaw.metrics.v1",
    ...
    if structured_results is not None:
        contract["structured_results"] = structured_results
    ...
    if primary is not None:
        score_payload = {
            ...
            "score": primary["score"],
            "model": "researchclaw",
            "details": f"source={source}; direction={metric_direction}",
            "source": "metrics.json",
            "generated": contract["generated"],
        }
        (target_dir / "best_score.json").write_text(
            json.dumps(score_payload, indent=2, ensure_ascii=False), encoding="utf-8"
        )
        (target_dir / "latest_score.json").write_text(
            json.dumps(score_payload, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    return contract


def _read_metrics_artifacts(directory: Path) -> tuple[dict[str, float], Any, list[str]]:
    """Read metrics from canonical and legacy experiment output files."""
    metrics: dict[str, float] = {}
    structured_results: Any = None
    sources: list[str] = []
    for name in ("metrics.json", "best_score.json", "latest_score.json", "results.json"):
        path = directory / name
        if not path.exists():
            continue
        ...
        if name == "results.json" and structured_results is None:
            structured_results = data
        if isinstance(data, dict):
            if name == "metrics.json":
                if isinstance(data.get("metrics"), dict):
                    metrics.update(_flatten_numeric_metrics(data["metrics"]))
                ...
                if isinstance(primary, dict) and isinstance(primary.get("score"), (int, float)):
                    ...
                    if math.isfinite(score):
                        metric_name = str(primary.get("name") or "primary_metric")
                        metrics.setdefault(metric_name, score)
                        metrics["primary_metric"] = score
                continue
            if isinstance(data.get("metrics"), dict):
                metrics.update(_flatten_numeric_metrics(data["metrics"]))
            metrics.update(_flatten_numeric_metrics(data))
    return metrics, structured_results, sources
...
def _collect_experiment_results(
    ...
    for stage_subdir in sorted(run_dir.glob("stage-*/runs")):
        contract_metrics, contract_structured, contract_sources = _read_metrics_artifacts(stage_subdir)
        if contract_structured is not None and structured_results is None:
            structured_results = contract_structured
        if contract_metrics:
            runs_data.append(
                {
                    "run_id": f"{stage_subdir.parent.name}/metrics-contract",
                    "task_id": "metrics-contract",
                    "status": "completed",
                    "metrics": contract_metrics,
                    "metric_sources": contract_sources,
    ...
    # Also scan stage roots. Stage 13 writes final refined metrics here.
    for stage_subdir in sorted(run_dir.glob("stage-*")):
        if not stage_subdir.is_dir():
            continue
        contract_metrics, contract_structured, contract_sources = _read_metrics_artifacts(stage_subdir)
        if contract_structured is not None and structured_results is None:
            structured_results = contract_structured
        if contract_metrics:
            runs_data.append(
                {
                    "run_id": f"{stage_subdir.name}/metrics-contract",
                    "task_id": "metrics-contract",
                    "status": "completed",
                    "metrics": contract_metrics,
                    "metric_sources": contract_sources,
