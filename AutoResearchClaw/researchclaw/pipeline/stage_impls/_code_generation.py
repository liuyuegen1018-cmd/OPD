...
def _execute_code_generation(
    ...
    *,
    ...
    # MAARS-inspired artifact contract: later stages must be able to read real
    # experiment numbers from files, not from prose.
    extra_guidance += (
        "\n\nMETRICS ARTIFACT CONTRACT (HARD REQUIREMENT):\n"
        "- main.py MUST write `metrics.json`, `best_score.json`, and `results.json` "
        "in the current working directory before exiting.\n"
        "- `metrics.json` MUST contain a top-level `metrics` object with finite "
        "numeric scalar values and a `primary_metric` object.\n"
        "- `best_score.json` MUST use this schema: "
        "`{\"metric\": \"<metric_name>\", \"score\": <number>, "
        "\"model\": \"<best_condition>\", \"details\": \"<short evidence>\"}`.\n"
        "- `results.json` SHOULD include per-condition and per-seed raw results "
        "so statistical summaries can be audited.\n"
        "- If no valid metric can be computed, exit with a non-zero status and "
        "print the cause. Do NOT exit code 0 with empty metrics.\n"
        "- Also print parseable metric lines: "
        "`condition=<name> seed=<s> {metric}: <value>` and one "
        "`METRIC_DEF: {metric} | direction=<higher/lower> | desc=<...>` line.\n"
        "- Do not fabricate metrics. Every value in these JSON files must be "
        "computed from actual experiment outputs in the script.\n"
    )
