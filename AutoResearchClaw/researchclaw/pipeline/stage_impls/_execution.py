...
def _latest_sandbox_project_dirs(sandbox_root: Path) -> list[Path]:
    if not sandbox_root.exists():
        return []
    return sorted(
        (p for p in sandbox_root.glob("_project*") if p.is_dir()),
...
def _execute_experiment_run(
    ...
    *,
    ...
    if mode in ("sandbox", "docker"):
        ...
        # Try to read structured metrics/results from the sandbox working dir.
        # ExperimentSandbox.run_project creates _project_1, _project_2, ...
        # while DockerSandbox exposes artifacts in its staging directory.
        structured_results: dict[str, Any] | None = None
        artifact_metrics: dict[str, float] = {}
        artifact_sources: list[str] = []
        artifact_dirs = _latest_sandbox_project_dirs(runs_dir / "sandbox")
        artifact_dirs.append(runs_dir / "sandbox")
        for artifact_dir in artifact_dirs:
            metrics_from_dir, structured_from_dir, sources = _read_metrics_artifacts(artifact_dir)
            if structured_from_dir is not None and structured_results is None:
                structured_results = structured_from_dir
                ...
                if results_json_path.exists():
                    try:
                        ...
                        pass
            if metrics_from_dir:
                artifact_metrics.update(metrics_from_dir)
                artifact_sources.extend(sources)
                for name in ("metrics.json", "best_score.json", "latest_score.json"):
        ...
        if artifact_metrics:
            effective_metrics.update(artifact_metrics)

        metrics_contract: dict[str, Any] | None = None
        if effective_metrics:
            metrics_contract = _write_metrics_contract(
                runs_dir,
                effective_metrics,
                metric_key=config.experiment.metric_key,
                metric_direction=config.experiment.metric_direction,
                source=";".join(artifact_sources) if artifact_sources else "stdout",
        ...
        elif result.returncode == 0 and not result.timed_out and not effective_metrics:
            run_status = "no_metrics"
            logger.warning(
                "Stage 12: Experiment exited successfully but produced no metrics. "
                "Generated code must write metrics.json/best_score.json/results.json "
        ...
        if metrics_contract is not None:
            run_payload["metrics_contract"] = metrics_contract
...
def _execute_iterative_refine(
    ...
    *,
    ...
    def _effective_rerun_metrics(sandbox_root: Path, stdout_metrics: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
        metrics: dict[str, Any] = dict(stdout_metrics or {})
        sources: list[str] = []
        for artifact_dir in _latest_sandbox_project_dirs(sandbox_root):
            metrics_from_dir, _structured, artifact_sources = _read_metrics_artifacts(artifact_dir)
            if metrics_from_dir:
                metrics.update(metrics_from_dir)
                sources.extend(artifact_sources)
                break
        if not metrics and stdout_metrics:
    ...
    if llm is None:
        ...
        if best_metric is None and config.experiment.mode != "simulated":
            contract_status = {
                "generated": _utcnow_iso(),
                "status": "failed",
                "reason": (
                    "No valid metrics were available and no LLM was available "
                    "to repair the experiment. The pipeline refuses to proceed "
                    "without real quantitative results."
                ),
                "metric_key": metric_key,
                "metric_direction": metric_direction,
                "stop_reason": "llm_unavailable_no_metrics",
            }
            (stage_dir / "metrics_contract_status.json").write_text(
                json.dumps(contract_status, indent=2), encoding="utf-8"
            )
            return StageResult(
                stage=Stage.ITERATIVE_REFINE,
                status=StageStatus.FAILED,
                artifacts=artifacts + ("metrics_contract_status.json",),
                error=contract_status["reason"],
                evidence_refs=tuple(
                    f"stage-13/{a}"
                    for a in artifacts + ("metrics_contract_status.json",)
                ),
            )
        return StageResult(
            stage=Stage.ITERATIVE_REFINE,
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if validation.ok and config.experiment.mode in ("sandbox", "docker"):
            ...
            sandbox_root = stage_dir / f"refine_sandbox_v{iteration}"
            rerun = sandbox.run_project(
                version_dir,
                timeout_sec=config.experiment.time_budget_sec,
            )
            effective_rerun_metrics, rerun_metric_sources = _effective_rerun_metrics(
                sandbox_root, rerun.metrics
            )
            metric_val = _find_metric(effective_rerun_metrics, metric_key)
            ...
            iter_record["sandbox"] = {
                ...
                "metrics": effective_rerun_metrics,
                "metric_sources": rerun_metric_sources,
            ...
            if (
                ...
                and not effective_rerun_metrics
            ):
                runtime_issues = (
                    (runtime_issues + "\n\n") if runtime_issues else ""
                ) + (
                    "METRICS CONTRACT FAILURE: The program exited with code 0 "
                    "but produced no parseable metrics. It MUST write "
                    "metrics.json and best_score.json, and should also write "
                    "results.json and print metric lines such as "
            ...
            if runtime_issues:
                ...
                if repaired_files:
                    ...
                    sandbox2_root = stage_dir / f"refine_sandbox_v{iteration}_fix"
                    rerun2 = sandbox2.run_project(
                        version_dir,
                        timeout_sec=config.experiment.time_budget_sec,
                    )
                    effective_rerun2_metrics, rerun2_metric_sources = _effective_rerun_metrics(
                        sandbox2_root, rerun2.metrics
                    )
                    metric_val = _find_metric(effective_rerun2_metrics, metric_key)
                    iter_record["sandbox_after_fix"] = {
                        "returncode": rerun2.returncode,
                        "metrics": effective_rerun2_metrics,
                        "metric_sources": rerun2_metric_sources,
                        "elapsed_sec": rerun2.elapsed_sec,
                        "timed_out": rerun2.timed_out,
                    }
                    iter_record["metric"] = metric_val
                    iter_record["runtime_repaired"] = metric_val is not None
                    if rerun2.returncode == 0 and not rerun2.timed_out and not effective_rerun2_metrics:
                        iter_record["runtime_repaired"] = False
                        iter_record["runtime_issues_after_fix"] = (
                            "METRICS CONTRACT FAILURE: repair made the program exit "
                            "cleanly but it still produced no metrics."
                        )

            if metric_val is not None:
                consecutive_no_metrics = 0
                metric_source = "stdout"
                current_metrics = effective_rerun_metrics
                if iter_record.get("sandbox_after_fix") and isinstance(
                    iter_record["sandbox_after_fix"], dict
                ):
                    after_fix_metrics = iter_record["sandbox_after_fix"].get("metrics")
                    if isinstance(after_fix_metrics, dict) and after_fix_metrics:
                        current_metrics = after_fix_metrics
                _write_metrics_contract(
                    stage_dir,
                    current_metrics,
                    metric_key=metric_key,
    ...
    if best_metric is None and config.experiment.mode != "simulated":
        contract_status = {
            ...
            "status": "failed",
            "reason": (
                "No valid metrics were produced after experiment execution and "
                "iterative refinement. The pipeline refuses to proceed to "
                "analysis/paper stages without real quantitative results."
            ),
            "metric_key": metric_key,
            "metric_direction": metric_direction,
            "stop_reason": log.get("stop_reason"),
            "iterations_completed": len(log.get("iterations", [])),
        }
        (stage_dir / "metrics_contract_status.json").write_text(
            json.dumps(contract_status, indent=2), encoding="utf-8"
        )
        artifacts.append("metrics_contract_status.json")
        return StageResult(
            stage=Stage.ITERATIVE_REFINE,
            status=StageStatus.FAILED,
